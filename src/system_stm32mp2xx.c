 ...
 *  2) each time HAL_RCC_ClockConfig() is called to configure the system
 *     clock frequency
...
void SystemCoreClockUpdate(void)
{
	...
	SystemCoreClock = xbar_source_freq /
			  (((RCC->FINDIVxCFGR[0] & RCC_FINDIVxCFGR_FINDIVx_Msk) + 1) *
			   ((RCC->PREDIVxCFGR[0] & RCC_PREDIVxCFGR_PREDIVx_Msk) + 1));
}
